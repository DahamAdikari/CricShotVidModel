...
def load_sequences(root):
    ...
    Returns:
      X: list of (T,33,5)
      y: list of class ids
      names: list of source file identifiers
    """
    X, y, names = [], [], []
...
def main():
    ...
    ap.add_argument("--T", type=int, default=32, help="Window length (frames)")
    ...
    X, y, names = load_sequences(args.pose_root)
    if not X:
        raise RuntimeError("No pose sequences found. Check pose_root and *_pose.npy files.")
    ...
    print("[INFO] Running final evaluation on validation split...")
    ...
    present_classes = sorted(set(yte.tolist()))
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["file", "true_label", "pred_label"] + [f"prob_{c}" for c in CLASSES])

        for name, yt, yp, probs in zip(nte, yte, y_pred, probs_all):
            writer.writerow([
                name,
                CLASSES[int(yt)],
                CLASSES[int(yp)],
    ...
    # confusion matrix
    cm = confusion_matrix(yte, y_pred, labels=present_classes)
    ...
    print(f"[OK] Saved report to: {report_dir / 'classification_report.txt'}")
    print(f"[OK] Saved predictions to: {report_dir / 'predictions.csv'}")
